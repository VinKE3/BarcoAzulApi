...
{
    public class rReporteProveedores
    {
        private readonly IEnumerable<oRegistroProveedor> _registros;
        private readonly oConfiguracionGlobal _configuracionGlobal;
        private readonly string _rptPath;

        public rReporteProveedores(IEnumerable<oRegistroProveedor> registros, oConfiguracionGlobal configuracionGlobal, string rptPath)
        {
            _registros = registros.OrderBy(x => x.Nombre);
            _configuracionGlobal = configuracionGlobal;
            _rptPath = Path.Combine(rptPath, "Sistema", "RptProveedores.rdl");
        }
        ...
        private byte[] Excel()
        {
            ...
            {
                ExcelWorksheet sheet = ep.Workbook.Worksheets.Add("Reporte de Proveedores");

                int row = 1, numeracion = 1;

                sheet.Cells[$"A{row}"].Value = "REPORTE DE PROVEEDORES";
