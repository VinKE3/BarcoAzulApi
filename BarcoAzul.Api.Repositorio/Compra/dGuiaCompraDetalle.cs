...
{
    public class dGuiaCompraDetalle : dComun
    {
        public dGuiaCompraDetalle(string connectionString) : base(connectionString) { }

        #region CRUD
        public async Task Registrar(IEnumerable<oGuiaCompraDetalle> detalles)
        {
            string query = @"   INSERT INTO Detalle_Compra (Conf_Codigo, Prov_Codigo, TDoc_Codigo, Com_Serie, Com_Numero, Com_Item, Suc_Codigo, DCom_AfectarStock,
                                ...
                                0, 0, @SubTotal, @PorcentajeIGV, @MontoIGV, 0, @Importe, 0, 0,
        ...
        public async Task Modificar(IEnumerable<oGuiaCompraDetalle> detalles)
        {
            string guiaCompraId = detalles.First().GuiaCompraId;

            await EliminarDeGuiaCompra(guiaCompraId);
            await Registrar(detalles);
        }

        public async Task EliminarDeGuiaCompra(string guiaCompraId)
        {
            var splitId = dGuiaCompra.SplitId(guiaCompraId);
        ...
        public async Task<IEnumerable<oGuiaCompraDetalle>> ListarPorGuiaCompra(string guiaCompraId)
        {
            var splitId = dGuiaCompra.SplitId(guiaCompraId);
            ...
            using (var db = GetConnection())
            {
                return await db.QueryAsync<oGuiaCompraDetalle>(query, new
                {
                    empresaId = new DbString { Value = splitId.EmpresaId, IsAnsi = true, IsFixedLength = true, Length = 2 },
                    proveedorId = new DbString { Value = splitId.ProveedorId, IsAnsi = true, IsFixedLength = true, Length = 6 },
                    tipoDocumentoId = new DbString { Value = splitId.TipoDocumentoId, IsAnsi = true, IsFixedLength = true, Length = 2 },
