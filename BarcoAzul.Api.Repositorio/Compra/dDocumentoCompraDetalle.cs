...
{
    public class dDocumentoCompraDetalle : dComun
    {
        public dDocumentoCompraDetalle(string connectionString) : base(connectionString) { }

        #region CRUD
        public async Task Registrar(IEnumerable<oDocumentoCompraDetalle> detalles)
        {
            string query = @"   INSERT INTO Detalle_Compra (Conf_Codigo, Prov_Codigo, TDoc_Codigo, Com_Serie, Com_Numero, Com_Item, Suc_Codigo, DCom_AfectarStock,
                                ...
                                VALUES
                                (@EmpresaId, @ProveedorId, @TipoDocumentoId, @Serie, @Numero, @DetalleId, '01', @AfectarStock,
                                @IngresoEgresoStock, @FechaEmision, @LineaId, @SubLineaId, @ArticuloId, @Descripcion, @UnidadMedidaId, @MonedaId, @Cantidad, @PrecioUnitario,
            ...
            using (var db = GetConnection())
            {
                foreach (var detalle in detalles)
                {
                    await db.ExecuteAsync(query, new
                    {
                        ...
                        IngresoEgresoStock = detalle.TipoDocumentoId == "07" ? "-" : "+",
        ...
        public async Task Modificar(IEnumerable<oDocumentoCompraDetalle> detalles)
        {
            string documentoCompraId = detalles.First().DocumentoCompraId;

            await EliminarDeDocumentoCompra(documentoCompraId);
            await Registrar(detalles);
        }

        public async Task EliminarDeDocumentoCompra(string documentoCompraId)
        {
            var splitId = dDocumentoCompra.SplitId(documentoCompraId);
        ...
        public async Task<IEnumerable<oDocumentoCompraDetalle>> ListarPorDocumentoCompra(string documentoCompraId)
        {
            var splitId = dDocumentoCompra.SplitId(documentoCompraId);
            ...
            string query = @"   SELECT
                                    ...
                                    DC.DCom_PrecioNeto AS SubTotal,
            ...
            using (var db = GetConnection())
            {
                return await db.QueryAsync<oDocumentoCompraDetalle>(query, new
                {
                    empresaId = new DbString { Value = splitId.EmpresaId, IsAnsi = true, IsFixedLength = true, Length = 2 },
                    proveedorId = new DbString { Value = splitId.ProveedorId, IsAnsi = true, IsFixedLength = true, Length = 6 },
                    tipoDocumentoId = new DbString { Value = splitId.TipoDocumentoId, IsAnsi = true, IsFixedLength = true, Length = 2 },
